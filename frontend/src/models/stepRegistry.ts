...
export const NODE_DEFINITIONS: NodeDefinition[] = [
  ...
  },
  {
    ...
    fields: [
      { name: 'key', label: '按键', type: 'text', required: true, placeholder: 'Enter / Escape / ArrowDown' },
    ],
  },
  {
    type: 'keyboardHotkey',
    ...
    group: 'page',
    formType: 'schema',
    defaultConfig: { modifiers: ['Control'], key: 'k' },
    fields: [
      { name: 'modifiers', label: '修饰键', type: 'text', placeholder: 'Control, Shift, Alt', extra: '多个修饰键用英文逗号分隔' },
      { name: 'key', label: '按键', type: 'text', required: true, placeholder: 'k / KeyP / Enter' },
    ],
  },
  {
    type: 'textExtract',
    ...
    group: 'data',
    formType: 'schema',
    defaultConfig: { input: '', pattern: '', groupIndex: 0, saveAs: '' },
    fields: [
      { name: 'input', label: '输入文本', type: 'textarea', required: true, placeholder: '${rawText}' },
      { name: 'pattern', label: '正则表达式', type: 'textarea', required: true, placeholder: 'order=(\\d+)' },
      { name: 'groupIndex', label: '分组序号', type: 'number', min: 0, extra: '默认 0，表示整段匹配；常用 1 提取首个捕获组' },
      { name: 'saveAs', label: '结果变量', type: 'text', required: true, placeholder: 'orderId' },
    ],
  },
  {
    type: 'goBack',
  ...
  },
  {
    ...
    group: 'page',
    formType: 'schema',
    defaultConfig: { keep: 'current', pageAlias: '' },
    fields: [
      { name: 'keep', label: '保留页面', type: 'select', options: [{ label: '当前页', value: 'current' }, { label: '指定别名', value: 'alias' }] },
      { name: 'pageAlias', label: '保留别名', type: 'text', placeholder: 'main', extra: '当保留页面为指定别名时必填' },
    ],
  },
  ...
  { type: 'listenRequestTrigger', label: '开始请求监听', color: '#1d4ed8', icon: '🛰️', group: 'wait', formType: 'schema', defaultConfig: { listenerId: '', urlPattern: '', matchType: 'contains', method: '', saveAs: '' }, fields: [{ name: 'listenerId', label: '监听器 ID', type: 'text', required: true, placeholder: 'loginApi' }, { name: 'urlPattern', label: 'URL 匹配', type: 'text', required: true, placeholder: '/api/login' }, { name: 'matchType', label: '匹配方式', type: 'select', options: [{ label: '包含', value: 'contains' }, { label: '完全匹配', value: 'equals' }] }, { name: 'method', label: 'HTTP 方法', type: 'text', placeholder: 'GET / POST，留空表示任意' }, { name: 'saveAs', label: '结果变量', type: 'text', placeholder: 'listenerRef' }] },
  { type: 'listenRequestResult', label: '读取监听结果', color: '#2563eb', icon: '📨', group: 'data', formType: 'schema', defaultConfig: { listenerId: '', saveAs: '', clearAfterRead: false }, fields: [{ name: 'listenerId', label: '监听器 ID', type: 'text', required: true, placeholder: 'loginApi' }, { name: 'saveAs', label: '结果变量', type: 'text', required: true, placeholder: 'requestSnapshot' }, { name: 'clearAfterRead', label: '读取后清空最新结果', type: 'radio', options: [{ label: '否', value: false }, { label: '是', value: true }] }] },
  { type: 'stopPageListen', label: '停止请求监听', color: '#1e3a8a', icon: '🛑', group: 'wait', formType: 'schema', defaultConfig: { listenerId: '', stopAll: false }, fields: [{ name: 'listenerId', label: '监听器 ID', type: 'text', placeholder: '留空时可配合 stopAll 停止当前页全部监听' }, { name: 'stopAll', label: '停止当前页全部监听', type: 'radio', options: [{ label: '否', value: false }, { label: '是', value: true }] }] },
  ...
  { type: 'saveData', label: '保存数据', color: '#0f766e', icon: '💾', group: 'data', formType: 'schema', defaultConfig: { content: '', fileName: '', saveDir: '', saveAs: '' }, fields: [{ name: 'content', label: '写入内容', type: 'textarea', required: true, placeholder: '${payload}' }, { name: 'fileName', label: '文件名', type: 'text', required: true, placeholder: 'result.json' }, { name: 'saveDir', label: '保存目录', type: 'text', placeholder: 'exports' }, { name: 'saveAs', label: '结果变量', type: 'text', placeholder: 'savedFilePath' }] },
  { type: 'saveExcel', label: '保存 Excel', color: '#15803d', icon: '📗', group: 'data', formType: 'schema', defaultConfig: { inputVar: '', fileName: '', saveDir: '', sheetName: 'Sheet1', saveAs: '' }, fields: [{ name: 'inputVar', label: '输入变量', type: 'text', required: true, placeholder: 'tableRows' }, { name: 'fileName', label: '文件名', type: 'text', required: true, placeholder: 'report.xlsx' }, { name: 'saveDir', label: '保存目录', type: 'text', placeholder: 'exports' }, { name: 'sheetName', label: '工作表名', type: 'text', placeholder: 'Sheet1' }, { name: 'saveAs', label: '结果变量', type: 'text', placeholder: 'excelPath' }] },
  { type: 'importExcel', label: '导入 Excel', color: '#65a30d', icon: '📘', group: 'data', formType: 'schema', defaultConfig: { path: '', sheetName: '', useHeader: true, saveAs: '' }, fields: [{ name: 'path', label: '文件路径', type: 'text', required: true, placeholder: 'data/report.xlsx' }, { name: 'sheetName', label: '工作表名', type: 'text', placeholder: '留空读取第一个工作表' }, { name: 'useHeader', label: '首行作为表头', type: 'radio', options: [{ label: '否', value: false }, { label: '是', value: true }] }, { name: 'saveAs', label: '结果变量', type: 'text', required: true, placeholder: 'excelRows' }] },
  { type: 'extractActiveElement', label: '提取焦点元素', color: '#9333ea', icon: '🎯', group: 'data', formType: 'schema', defaultConfig: { extractType: 'value', saveAs: '' }, fields: [{ name: 'extractType', label: '提取内容', type: 'select', options: [{ label: 'value', value: 'value' }, { label: '文本', value: 'text' }, { label: 'HTML', value: 'html' }, { label: '标签名', value: 'tagName' }] }, { name: 'saveAs', label: '结果变量', type: 'text', required: true, placeholder: 'activeValue' }] },
  { type: 'getClipboardText', label: '读取剪贴板', color: '#0ea5e9', icon: '📋', group: 'data', formType: 'schema', defaultConfig: { saveAs: '' }, fields: [{ name: 'saveAs', label: '结果变量', type: 'text', required: true, placeholder: 'clipboardText' }] },
  ...
  { type: 'convertJson', label: '转换 JSON', color: '#0f766e', icon: '🧱', group: 'data', formType: 'schema', defaultConfig: { sourceVar: '', targetFormat: 'object', outputVar: '' }, fields: [{ name: 'sourceVar', label: '输入变量', type: 'text', required: true, placeholder: 'rawJson' }, { name: 'targetFormat', label: '转换目标', type: 'radio', options: [{ label: '转对象', value: 'object' }, { label: '转字符串', value: 'string' }] }, { name: 'outputVar', label: '输出变量', type: 'text', required: true, placeholder: 'parsedJson' }] },
  { type: 'extractKey', label: '提取键值', color: '#0369a1', icon: '🗝️', group: 'data', formType: 'schema', defaultConfig: { sourceVar: '', keyPath: '', outputVar: '' }, fields: [{ name: 'sourceVar', label: '输入变量', type: 'text', required: true, placeholder: 'payload' }, { name: 'keyPath', label: '键路径', type: 'text', required: true, placeholder: 'data.items[0].title' }, { name: 'outputVar', label: '输出变量', type: 'text', required: true, placeholder: 'firstTitle' }] },
  { type: 'randomGet', label: '随机取值', color: '#a16207', icon: '🎲', group: 'data', formType: 'schema', defaultConfig: { sourceVar: '', outputVar: '' }, fields: [{ name: 'sourceVar', label: '数组变量', type: 'text', required: true, placeholder: 'items' }, { name: 'outputVar', label: '输出变量', type: 'text', required: true, placeholder: 'randomItem' }] },
  ...
  { type: 'callWorkflow', label: '调用子流程', color: '#7c2d12', icon: '🪄', group: 'flow', formType: 'custom', defaultConfig: { workflowId: '', inputMapping: {}, outputVar: '' } },
];
