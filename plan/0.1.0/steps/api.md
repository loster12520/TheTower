...
导语：本文档用于冻结 0.1.0 的接口、事件与错误码扩展方向，确保体验增强、高级调试、第三方集成和产品化能力在进入开发前有统一协议基线。

## 1. 目标与边界

- 目标：补齐 0.1.0 新能力需要的 REST、WebSocket、错误码和数据模型定义。
- 目标：避免出现前端入口已上线但后端协议未冻结，或后端接口已实现但文档缺失的情况。
- 目标：为调试状态机、调度任务、权限体系和第三方接入提供可拆分的协议骨架。
...
| 外部平台集成 | 请求监听、Google Sheet、邮箱、验证码平台、LLM 接入所需配置模型 |
...
- 所有新增接口必须给出稳定业务码，不直接向前端暴露第三方原始异常。
- 第三方集成必须区分“凭证错误”“限流错误”“网络错误”“配置缺失”四类失败原因。
