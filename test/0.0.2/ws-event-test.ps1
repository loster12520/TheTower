...
$requestId='ws-test-0.0.2'

$tplBody = @{
  name='ws-test-template-0.0.2'
  description='ws test'
  schemaVersion='0.0.1'
  steps=@(
...
$runStart = Invoke-RestMethod -Uri "$base/runs" -Method Post -Headers $headers -ContentType 'application/json' -Body $startBody
...
while ((Get-Date) -lt $deadline -and $client.State -eq [System.Net.WebSockets.WebSocketState]::Open) {
  ...
  if (-not $receiveTask.Wait(1200)) { continue }
...
$requestIdMatched = @($messages | Where-Object { $_.requestId -eq $requestId }).Count -gt 0
...
$summary = [pscustomobject]@{
  ...
  checks = [pscustomobject]@{
    ...
    requestIdPropagated = $requestIdMatched
  }
}
